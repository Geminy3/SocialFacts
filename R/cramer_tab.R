...
cramer_tab <- function(var = NULL, metric = 'cramer', data = NULL, weight = NULL) {
  ...
  for (x in var) {
    for (y in var) {
      if (x != y) {
        ...
        if (metric == 'cramer') {
          N = length(data[[x]])
          chi2 <- weights::wtd.chi.sq(var1 = data[[x]], var2 = data[[y]], weight = weight)
          Phi = chi2[["Chisq"]]/N
          Row = length(unique(data[[x]]))
          C = length(unique(data[[y]]))
          Cramer.V = sqrt(Phi/min(Row - 1, C - 1))
          vec_metric[[x]][y] <- round(Cramer.V, 5)
          vec_chi[[x]][y] <- round(chi2[["p.value"]], 5)
        } else if (metric == 'cor') {
          vec_metric[[x]][y] <- stats::cor(data[[x]], data[[y]])
        }
      } else {
        vec_metric[[x]][y] <- NA
      }
