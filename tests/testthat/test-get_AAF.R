test_that("get_AAF works", {
  ...
  #print(str(df))
  model <- glm(var_dep ~ var_1 + var_2, data = data, family = binomial(link="logit"))
  expect_no_failure(get_AAF(model = model, nvar = 3, vars_dep = "var_dep", data = data, nbootstrap = 5))
  #print(res)
})
